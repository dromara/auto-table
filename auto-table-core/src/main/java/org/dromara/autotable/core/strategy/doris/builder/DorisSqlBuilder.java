...
public class DorisSqlBuilder {
    ...
    public static String buildSql(DorisTableMetadata tableMetadata) {
        ...
        String keys = tableMetadata
                .getKeys()
                .stream()
                .map(column -> "`" + column + "`")
                .collect(Collectors.joining(", "));
        String tableComment = StringUtils.hasText(tableMetadata.getComment())
                ? "comment '" + tableMetadata.getComment() + "\"" : "\"";

        // String properties = "properties (" + String.join(",", tableMetadata.getProperties()) + ")";
