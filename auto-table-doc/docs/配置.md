...
* **单数据源场景（没有指定数据源的情况）**
  * sql/[表名].sql -> 指定[表名]的相关表的脚本（只会在指定表创建完成后执行一次）
  * sql/[_init_].sql -> 表示整个库的相关脚本（只会在库创建后，且所有表创建完成后执行一次）
* **多数据源场景（有多套数据库，每个库属于一个数据源）**
  * sql/[数据源名称]/[表名].sql -> 表示[数据源名称]下的指定[表名]的相关表脚本（只会在指定表创建完成后执行一次）
  * sql/[数据源名称].sql -> 表示[数据源名称]下整个数据库的相关脚本（只会在库创建后，且所有表创建完成后执行一次）
