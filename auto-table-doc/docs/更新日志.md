...
## 2.6.2.1
* chore(release): 补发 v2.6.2 adapter 模块
* chore(release): 删除过期的 2.6.1 版本发布日志文件
* feat(adapter): 启用 Adapter 模块 Maven Central 发布
* docs(changelog): 更新版本 2.6.3 的变更日志
* docs(changelog): 更新版本 2.6.3 的变更日志
* docs(orm-adaptation): 修正ORM适配文档中迁移指南链接格式
* docs(migration): 移除文档中指向本地文件的无效迁移指南链接
* chore(project): 清理废弃的迁移文档及Anyline测试依赖
* docs: 修正 ORM 适配总览页 content，添加完整的介绍页面
* docs: 修正 ORM 适配目录下 MyBatis-Plus.md 文件内容
* docs: 使用硬链接方式共享 ORM 适配和框架集成的 MyBatis-Plus/Flex 文档
* docs: 创建 ORM 适配目录的软链接，修复页面路径
* docs: 创建独立的 ORM 适配菜单，将 MyBatis-Plus/Flex 提升为顶级导航
* docs: 重写 MP/MF适配器文档，强调零注解自动建表的核心价值
* docs: 修正框架集成文档，分离 MP 和 MF 升级说明
* docs: 完善 2.6.2 版本文档，补充 MyBatis-Flex 集成指南和迁移说明
* test: update test database files
* docs(changelog): 更新版本 2.6.2 的变更日志
* feat(adapter): 新增 MyBatis-Flex 适配器，重构 MyBatis-Plus 适配器
* docs(database): 更新数据库支持文档和依赖配置
* fix(solon): support Solon 4.0.3
* chore: 简化 upgrade.sh 为全量发布，依赖 skipPublishing 过滤

## 2.6.2

### 🎉 新功能

#### 1. 新增 MyBatis-Flex 适配器

全新支持 [Mybatis-Flex](https://gitee.com/mybatis-flex/mybatis-flex) 框架，让 Mybatis-Flex 用户也能享受自动建表的便利。
...
详见：[MyBatis-Flex 集成文档](/框架集成/Mybatis-Flex)

### 🔧 重大变更

#### MyBatis-Plus 适配器重构

移除了 MyBatis-Plus 适配器的扩展注解体系，回归零侵入设计理念：
...
**迁移建议：**
所有使用过扩展注解的用户请查看 [迁移指南](file:///Users/don/Code/个人/auto-table/MIGRATION-FROM-MYBATIS-PLUS-EXT.md)，改用标准注解即可。

### 📚 文档完善

- 补充 MyBatis-Flex 集成详细教程
- 添加迁移指南文档
- 完善常见问题章节
- 更新数据库支持列表
...
- 简化 release.sh 脚本为全量发布模式
- 依赖管理采用 `skipPublishing` 过滤机制
- auto-table-test 模块已配置跳过发布（已在 pom.xml 中标记）
