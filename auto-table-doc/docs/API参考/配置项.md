...
::: tip 提示
`update`模式不会删除字段(更改字段名称的情况下，会认为是新增字段)，如果需要从数据库强制删除实体上不存在的字段，请参考 [auto-drop-column](/API参考/配置项#auto-drop-column)
配置项。
:::
...
#### alter-table-separate-drop <Badge type="warning" text="^2.5.10" />

- 类型：`Boolean`
- 默认：`false`

修改表的时候，是否分离删除类的sql，通常适用于云数据库的安全限制，需要把alter table的删除类sql单独执行
